# AISec LabRunner, Module 1: Data Ingestion, Hidden Leakage, Reproducible Pipelines

Three interactive labs on a synthetic SOC environment (`acme-soc`) with a private week-2 gate.
Runtime-agnostic: a step/probe harness runs from the command line today and exposes a JSON manifest for an interactive UI
platform to wire against.

## 1. Setup

Requirements: Python 3.11 (course standard; tested on 3.12), 16 GB RAM, about 1 GB disk for generated data.

**Check your interpreter first.** `python --version` on a SIFT workstation returns 3.8.10, which is end of life and
cannot install this package's requirements (numpy 1.26 needs 3.9+). Use a 3.11 virtual environment:

```
sudo apt install software-properties-common
sudo add-apt-repository ppa:deadsnakes/ppa && sudo apt update
sudo apt install python3.11 python3.11-venv python3.11-dev
python3.11 -m venv ~/venv/aisec && source ~/venv/aisec/bin/activate
pip install --upgrade pip && pip install -r requirements.txt
python prepare.py                 # generates both weeks (about 90 s), builds the Lab 2 tables (about 40 s), writes the manifest
```

If you cannot install 3.11, `requirements-py38.txt` pins the last releases that support 3.8. That path is untested
by the author. Smoke-test it before class:

```
pip install -r requirements-py38.txt
python prepare.py --scale 0.2 && python run_lab.py --lab 1 --all --solution && python run_lab.py --lab 3 --all --solution
```

`prepare.py --scale 0.2` builds a 20% environment for authoring and smoke tests. Thresholds in `harness/thresholds.json`
...
harness/probes.py         one probe per step, all feedback text in MESSAGES
harness/manifest.py       emits harness/lab_manifest.json (step graph, decision points, state contract, branches)
harness/thresholds.json   week-2 pass thresholds
CAST.md                 who everyone in the environment is; hand this to learners with Lab 1
...
change `W1` handling in `common.py` and `harness/probes.py`.
...
`STOP` means the step is not accepted; the branch id in parentheses matches `lab_manifest.json`.

## 4. Wiring to the AISec LabRunner UI (or any interactive runtime)

`harness/lab_manifest.json` is the integration surface. For each lab it lists steps, the decision prompt and options to
...
2. After the learner submits a step, call `harness.probes.PROBES[(lab, step)](state)` and render `Result.branch` and `Result.message`.
...
`--all --solution` and recalibrate `harness/thresholds.json`.

## 5. Measured numbers, full scale, seed 20260901

These replace the design targets in the specification. PR-AUC throughout. Regenerated after the named-people and
CMDB additions; rerun both reference solutions after any generator change.
...
| Lab 2, honest spec, week-1 random CV | 0.59 |
...
| Named staff / hosts / SOC analysts in the environment | 25 / 40 / 5 |
...
**Lab 2 (in class, 35 minutes).** The pool has nine leaks and one misleading legitimate feature. Any one of
...
`cmdline_window` is the one most SOC engineers will recognise from their own environment: the symmetric window pulls in
Callum Reid's and Ifeoma Eze's containment commands, so the model learns the response rather than the attack.
`raised_by` and `department` are the two human-shaped leaks: an analyst name exists only where a ticket exists, and
`department = "unmanaged"` means the CMDB never heard of the machine, which is exactly the red-team workstation. The full-data target-encoding
